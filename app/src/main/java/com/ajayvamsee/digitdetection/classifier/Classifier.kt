...
class Classifier(
    ...
    init {
        Log.v("TAG", "[Input] shape = ${inputTensor.shape()?.contentToString()}" + "dataType= ${inputTensor.dataType()}")
        Log.v("TAG", "[Output] shape = ${outputTensor.shape()?.contentToString()}, " +
    ...
    fun classify(image:Bitmap):Recognition{
        ...
        Log.v("TAG", "classify(): timeCost = $timeCost, top = $top, probs = ${probs.contentToString()}")
